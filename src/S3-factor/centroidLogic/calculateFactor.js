...
const calculateFactor = (reflectedArray, columnTotals) => {
  ...
  factorLoad1 = map(colTotalsAndMeanSum, (num) =>
    evenRound(num / totalsSumsSqrt, 8)
  );
  ...
  function totalSumsFunction(newDiagonalEstimate) {
    totalsSums = reduce(newDiagonalEstimate, (sum, num) =>
      evenRound(sum + num, 8)
    );
    return totalsSums;
  }
  ...
  function factorLoad1Function(newDiagonalEstimate) {
    factorLoad1 = map(newDiagonalEstimate, (num) =>
      evenRound(num / totalsSumsSqrt, 8)
    );
    return factorLoad1;
  }
  ...
  if (maxDiff > 0.001) {
    do {
      ...
      diffDiagonalEstimateandFactorLoad = [];
      for (
        let m = 0, mLen = previousFactorLoadEstimate.length;
        m < mLen;
        m += 1
      ) {
        diffDiagonalEstimateandFactorLoad.push(
