...
const loadingsTableDataPrep = (numFactors) => {
  ...
  const fSigCriterionResults = S5DataSlice.getState().fSigCriterionResults;

  // calculate the factor groupings so they can be assigned in col defs
  const highlighting = S5DataSlice.getState().highlighting;

  const NumTrans = "Number";
  ...
  // set up state for loadings table
