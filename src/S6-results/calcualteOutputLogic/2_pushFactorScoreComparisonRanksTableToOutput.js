...
const pushFactorScoreComparisonRanksTableToOutput = function (
  ...
  // getState
  const synFactorArray1 = S6DataSlice.getState().synFactorArray1Holder;
  ...
  let sheetName1 = "Factor Score Ranks";
  if (sheetName1.length > 30) {
    sheetName1 = "Factor Score Ranks short";
  }
  ...
  const compositeFactorMasterArray = cloneDeep(
  ...
  for (let yy = 0, yyLen = userSelectedFactors.length; yy < yyLen; yy++) {
    ...
    const factorName = `${"Factor"} ${factorNum}`;
  ...
  S6DataSlice.setState({ factorScoreRanksArray: factorScoreRanksArray });
  ...
  S6DataSlice.setState({ statementRankingArray: statementRankingArray });
