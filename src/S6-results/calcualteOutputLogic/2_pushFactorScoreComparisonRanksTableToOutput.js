...
const pushFactorScoreComparisonRanksTableToOutput = function(
  ...
  for (var j = 0, jLen = synFactorArray1.length; j < jLen; j++) {
    ...
    synFactorArray1[j] = orderBy(
      synFactorArray1[j],
      [obj => obj["Z-score"], obj => obj["Statement Number"]],
      ["desc", "asc"]
    );
    ...
    //  sort function to put table in proper order after assigning ranks
    synFactorArray1[j].sort(function(a, b) {
  ...
  for (
    ...
    compositeFactorMasterArray[kk] = orderBy(
