...
const pushFactorsToOutputArray = (outputData, sheetNamesXlsx, colSizes) => {
  ...
  const results = cloneDeep(S5DataSlice.getState().currentLoadingsTable);
  ...
  if (multipleFactorsFlaggedArray.length > 0) {
    window.multipleFactorsFlaggedWarningModal.showModal();

    console.log("multiple factors flagged for: ", multipleFactorsFlaggedArray);
  ...
  //   outputState.sortsFlaggedOnTwoFactors = flaggedArrayText;
  //   outputState.showMultipleFactorsFlaggedWarningModal = true;
  ...
  if (problemFactorsArray.length > 0) {
    ...
    console.log("process halted - factors without flag selected");
    return "haltOutputProcessing";
  }
  ...
  const analysisOutput2 = cloneDeep(S6DataSlice.getState().analysisOutput);
  const analysisOutput = cloneDeep(analysisOutput2);
  const sigSortsArray = cloneDeep(S6DataSlice.getState().sigSortsArray);
  const sortsAsNumbers = cloneDeep(S6DataSlice.getState().sortsAsNumbers);
  const qavRespondentNames = cloneDeep(S1DataSlice.getState().respondentNames);
  const correlationTableArray = cloneDeep(
  ...
  const sortWeights = cloneDeep(S6DataSlice.getState().sortWeights);
  ...
  for (let i = 0; i < analysisOutput.length; i += 1) {
    ...
    const factorNumber = `${"Fac"} ${number}`;

    // get trans. - MS Excel has 30 char. limit, (prepend FAC. #)
    let sheetName1 = "Sorts Weight";
    if (sheetName1.length > 23) {
      sheetName1 = "Sorts Weight short";
    }
    ...
    // get trans. - MS Excel has 30 char. limit, (prepend FAC. #)
    let sheetName2 = "Sorts Corr";
    if (sheetName2.length > 23) {
      sheetName2 = "Sorts Corr short";
    }
  ...
  for (let j = 0; j < analysisOutput.length; j += 1) {
    ...
    const factorWeightFactorArray = [["Nm", "Q Sort", "Weight"]];
    ...
    for (let m = 0, mLen = analysisOutput[0].length; m < mLen; m += 1) {
      ...
      for (let s = 0; s < sLen; s += 1) {
        tempObj[`${"Raw Sort"} ${sigSortsArray[j].SigSorts[s]}`] =
          rawSorts[j][s][m];
