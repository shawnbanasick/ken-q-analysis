...
const formatConsensusArrayForDownload = (
  ...
  // getState
  const distStateThreshold1 = S6DataSlice.getState().distStateUpperValueText;
  const distStateThreshold2 = S6DataSlice.getState().distStateLowerValueText;

  const chartText1 = "Consensus Statements";
  const chartText2 =
    "Those That Do Not Distinguish Between ANY Pair of Factors";
  const chartText3 = `${"All Listed Statements are Non-Significant at"} ${distStateThreshold1}${"and Those Flagged with an are also Non Significant at"} ${distStateThreshold2})`;
