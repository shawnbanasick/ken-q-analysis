...
// todo - bug fix 2/2016 - manual input not working - no clearing or registering of data
...
$(document).ready(function () {
    ...
    //todo - check to see if this is still active and needed
    $("#manualInputDatabaseSelect").change(function () {
        var databaseSelected = $(this).val();
        if (databaseSelected === "new") {

        } else {
            localStorage.setItem("manualInputDatabase", databaseSelected);
        }
    });
...
// todo - clean up and reduce document ready functions
...
function exportSortsExcel() {
    ...
    var projectName2 = sanitizeUserInputText(projectName3);
...
function sanitizeRespondentName(e) {
    e = e.replace(/ /g, "_");
    if (e.length > 8) {
        e = e.substring(0, 8);
    }
...
function beginMIAnalysis() {
    ...
    var qavProjectName = sanitizeUserInputText(qavProjectName1);
...
function sanitizeUserInputText(input) {
...
function checkSortCondition(duplicates, missingIncomplete, goodInput2) {
    ...
    if (duplicates === true && missingIncomplete === true && goodInput2 === "true") {
        ...
        var respondentName1 = sanitizeUserInputText(respondentName);
